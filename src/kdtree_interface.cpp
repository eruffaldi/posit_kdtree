...
public:
	...
	typedef nanoflann::KDTreeSingleIndexAdaptor<
     ...
     virtual int knnSearch(int K, const float * query_point_f,size_t * output)  const
     {
     	if(!index)
     		return 0;
     	auto num_results = K;
     ...
     virtual int radiusSearch(float asearch_radius, const float * query_point_f,  int num_results, int * output)  const
     {
     	if(!index)
     		return 0;
